 ...
 <div class="div-warp">
      ...
      <el-dialog
        ...
        >
                ...
                <el-form-item label="负责人姓名" prop="manager" size="small" :label-width="formLabelWidth">
                  <el-input v-model="editForm.manager" auto-complete="off" :disabled="true"></el-input>
                </el-form-item>
                <el-form-item label="联系方式" size="small" :label-width="formLabelWidth">
                  <el-input v-model="editForm.mobile" auto-complete="off" :disabled="true"></el-input>
                </el-form-item>
      ...
      <div class="search-warp">
        <el-row :gutter="20" >
            ...
            <el-col :span="16">
                <div class="grid-content bg-purple" >  
                  <el-button type="success"  size="small" @click.native.prevent="search" v-permission="['sys:point:district:select']">查询</el-button>
                  <el-button type="primary" size="small" @click="handleAdd" v-permission="['sys:point:district:add']">新增</el-button>
                  <el-button type="danger" size="small" @click="deleteRow" v-permission="['sys:point:district:del']">删除</el-button>
                  <el-upload
                  style="display:inline-block; margin:0 8px;"
                  accept=".xls"
                  action="/system/vendingDistrict/importExcel"
                  :show-file-list="false"
                  multiple
                  :on-success="handleSuccess"
                  >
                  <el-button size="small" type="warning" v-permission="['sys:point:district:edit']">导入</el-button>
                  
                </el-upload>
                <el-button size="small" type="primary" @click="downLoadModel" v-permission="['sys:point:district:edit']">下载模板</el-button>
                <el-button size="small" type="warning" @click="exportExcelFile" v-permission="['sys:point:district:edit']">导出</el-button>
                </div>
            </el-col>
          ...
          width="150">
            <template  scope="scope">
              <div style="text-align:center;">
                <el-button type="success" size="mini" icon="el-icon-view" circle @click="handdetails(scope.row)" v-permission="['sys:point:district:view']"></el-button>
                <el-button type="primary" size="mini" icon="el-icon-edit" circle @click="handleEdit(scope.$index, scope.row)" v-permission="['sys:point:district:edit']"></el-button>
                <el-button type="danger"  size="mini" icon="el-icon-delete" circle @click.native.prevent="deleteRow(scope.row)" v-permission="['sys:point:district:del']"></el-button>
              </div>
            </template>
        </el-table-column>
      </wTable>
...
<script>
import { getList,delArea,exportDistrictExcel,downLoadDistrictExcelModel } from '@/api/pointposition'
...
export default {
  ...
  methods: {
    ...
    exportExcelFile(){
      ...
      exportDistrictExcel(para).then((res) => {
    ...
    downLoadModel(){
      ...
      NProgress.start();
      var para = {}
      downLoadDistrictExcelModel(para).then((res) => {
        if(res.zhead.reTCode=="0000"){
    ...
    handleSelectionChange(rows) {//全选
        ...
        if (rows) {
          rows.forEach(row => {
             _this.multipleSelection += row.districtId+ ",";
    ...
    deleteRow(row) {//删除数据
      ...
      this.$confirm('确认提交吗？', '提示', {}).then(() => {
                  let para ={
                    ids:_this.multipleSelection =="" ? row.districtId : _this.multipleSelection
                  }
                  delArea(para).then((res) => {
                        if(res.zhead.reTCode=="0000"){
                        NProgress.done();
                        this.$notify({
                          title: '成功',
