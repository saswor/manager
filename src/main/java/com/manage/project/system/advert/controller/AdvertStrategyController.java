 ...
 */
@Controller
@RequestMapping("/system/advertStrategy")
public class AdvertStrategyController extends BaseController
{
	...
	@GetMapping("/list")
	@ResponseBody
	public AjaxResult list(AdvertStrategy advertStrategy)
	{
		startPage();
		if (SystemUtil.isZhilai()) {
			advertStrategy.setCorpId("");
		} else {
			advertStrategy.setCorpId(ShiroUtils.getUser().getCorpId());
		}
	...
	{		
		return toAjax(advertStrategyService.deleteAdvertStrategyByIds(ids.getIds()));
	}
