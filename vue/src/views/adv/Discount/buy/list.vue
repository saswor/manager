                ...
                <el-row align="middle"  :gutter="10" class="m-top">
                  ...
                  <el-col :span="14">
                     <div v-for="item in infoData.ftimes" class="tiems">
                       ...
                       <span><em>折扣：</em>{{item.discount}}</span>
                     </div>
                  </el-col>
                </el-row>
                 ...
                 <el-form :model="addForm" :rules="Rules" ref="addForm"  label-width="200px">
                    <el-tabs v-model="activ" @tab-click="handleClicka">
                      <el-tab-pane label="基本信息" name="set1" disabled>
                        ...
                        <el-form-item label="折扣：" prop="discount" size="small" :label-width="formLabelWidth" v-if="addForm.favType=='1'">
                            <div style="display:inline-block;float:left; margin-right:5px;" v-for="(z,index) in addForm.ftimes">

                            <el-input style="width:50px; text-align:center;margin-right:5px;"  v-model="z.discount"  ></el-input>折
                            </div>
                            

                        </el-form-item>

                        <el-form-item label="优惠时间段："  v-else size="small" :label-width="formLabelWidth">
                            <div style="display:inline-block;float:left; margin-right:5px;" v-for="(z,index) in addForm.ftimes">
                                <el-time-picker
                                  v-model="z.favSTime"
                            ...
                            <el-input style="width:100px;" placeholder="折扣"  v-model="z.discount"></el-input>
                 ...
                 <el-form :model="editForm" :rules="Rules" ref="editForm"  label-width="200px">
                    <el-tabs v-model="activ" @tab-click="handleClicka">
                      <el-tab-pane label="基本信息" disabled name="set1">
                        ...
                        <el-form-item prop="validSTime" label="有效时间段" size="small" :label-width="formLabelWidth">
                                     <el-date-picker
                        ...
                        <el-form-item label="折扣：" prop="discount" size="small" :label-width="formLabelWidth" v-if="editForm.favType=='1'">
...
export default {
  ...
  created(){
    if(this.$route.query.favTarget==1){
      ...
      this.titlesList=['选择售卖机','下发的售卖机']
    }else{
      this.labelText= "选择商品"
      this.labelSeeText = "选择商品信息"
      this.titlesList=['选择商品','下发的商品']
    }
  },
  methods: {
    ...
    deleteRow(row) {//删除数据
      ...
      this.$confirm('确认提交吗？', '提示', {}).then(() => {
                  let para ={
                    ids:_this.multipleSelection=="" ? row.favourableId : _this.multipleSelection
                  }
          ...
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
                      ...
                      if(formName=="addForm"){
                          _this.adevicesValue.forEach((item, index) => {
                              ...
                              favType:_this.$route.query.favTarget,
...
<style scoped>
  ...
  .m-top{ line-height:40px; border-bottom:1px solid #efefef}
  .m-top strong{ display:block; text-align:right}
