...
<script>
import {updateVendingVersion,getVendingVersionDetails} from '@/api/equipment'
...
export default {
  ...
  methods: {
    getVendingVersionDetailsData(page){
        var _this = this;
        const listQuery = {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    ...
                    updateVendingVersion(para).then((res) => {
  ...
  mounted () {
    this.getVendingVersionDetailsData();
  },
  components: {
