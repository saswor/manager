...
<script>
import {editRole,getMenuList,getRoleMenuList,getRoleiSMenuList} from '@/api/systemSetup'
...
export default {
  ...
  created() {
      ...
      const listQuery = {
       roleIds:this.addForm.roleId
      }
      getRoleMenuList(listQuery).then(response => {
         if(response.zhead.reTCode==="0000"){
          NProgress.done();
          this.addForm.menuIds = response.zbody.datas;
  ...
  methods: {
    ...
    getRoleiSMenuData(){
      var _this = this;
      const listQuery = {
        roleId:_this.addForm.roleId
      }
      getRoleiSMenuList(listQuery).then(response => {
         if(response.zhead.reTCode==="0000"){
          NProgress.done();
          _this.addForm.menuIds = response.zbody.datas;
         }else{
             this.$notify({
              title: '失败',
              message: res.zhead.retMsg,
